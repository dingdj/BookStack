tinymce.PluginManager.add('smileys', function (editor, url) {
    ...
    var smileys = editor.settings.smileys || defaultSmileys,
        fullSmileysList = editor.settings.extended_smileys ? smileys.concat(editor.settings.extended_smileys) : smileys;
    ...
    var fullSmileyArray = concatArray(fullSmileysList);
    ...
    function replaceAllMatches(smiley, node) {
        if (typeof (smiley.shortcut) === 'string') {
            replaceEmoticon(smiley, node);
        } else if (Array.isArray(smiley.shortcut)) {
            tinymce.each(smiley.shortcut, function (shortcutString) {
                let smileyObj = {'shortcut': shortcutString, 'url': smiley.url};
                return replaceEmoticon(smileyObj, node);
            });
        }
    }

    function replaceEmoticon(smiley, node) {
        if (typeof (smiley.shortcut) === 'string') {
            if (!node.textContent.endsWith(smiley.shortcut)) { //fast check condition
                return;
            }
            let text = smiley.shortcut.replace(/[\-\[\]\/\{\}\(\)\*\+\?\.\\\^\$\|]/g, "\\$&");
            let marker = editor.dom.create('img', {"src": smiley.url, "title": smiley.title});
    ...
    editor.on("keyup", function (e) {
        if (!!editor.settings.auto_convert_smileys) {
            ...
            if (node) {
                tinymce.each(fullSmileyArray, function (smiley) {
                    replaceAllMatches(smiley, node);
                });
            }
        }
    });
